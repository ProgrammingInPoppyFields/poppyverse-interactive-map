...
<body>
  ...
  <script>
    ...
    const AXIS_SCALE = 560;     // max reach of Linear Time (x2, since it starts at 0) and the Maturity Depth/Multiverse Stability orbit radius
    ...
    function prepareGraphData() {{
      ...
      DATA.nodes.forEach(node => {{
        ...
        if (node.isTrailer) {{
          node.x = rand(seed + 10) * 2 * AXIS_SCALE;
        ...
        node.x = relN * (AXIS_SCALE * 2) * (1 + (rand(seed) - 0.5) * JITTER) + jx;
        node.y = radius * Math.cos(theta) + jy;
        node.z = radius * Math.sin(theta) + jz;
