...
<body>
  ...
  <script>
    ...
    function renderCoordMeters(node) {{
      // Final Y/Z are the Cartesian PROJECTION of Maturity (radius) and
      // Stability (angle) -- y = r*cos(theta), z = r*sin(theta) -- so
      // neither one alone IS Maturity or Stability, and showing them raw
      // is exactly what made "negative Maturity/Stability" look meaningless
      // (they're not scores at all, just a rotated point's coordinates).
      // Converting back to polar here fixes that: radius via hypot() can't
      // be negative (it's a distance), and the angle is normalized to
      // 0-360 degrees, matching the ring in the 3D scene.
      const y = Number(node.finalY) || 0;
      const z = Number(node.finalZ) || 0;
    ...
    function start() {{
      ...
      // 90/270 sit right on the Multiverse Stability axis line (colored to
      ...
      [0, 90, 180, 270].forEach(deg => {{
        ...
        const label = makeAxisLabel(deg + "°", tickColors[deg], {{ fontPx: 22, opacity: 0.7, spriteHeight: 24 }});
        label.position.set(tickXOffset[deg] || 0, tickRadius * Math.cos(t), tickRadius * Math.sin(t));
        axesGroup.add(label);
      }});
